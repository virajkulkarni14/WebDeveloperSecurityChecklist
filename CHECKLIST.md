...
- [ ] Don’t keep port 22 open on any AWS service groups on a permanent basis. Instead consider only allowing authorised IPs to SSH on the box, by whitelisting them, for example with iptables. Disable root SSH access and consider relying on Public-Key Authentication only if SSH access is not IP restricted.
